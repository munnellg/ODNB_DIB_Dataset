...
class Entity:
    ...
    def __parse_date_part(self, date_part):
        date = re.search(r"[0-9/]+", date_part)
        if date == None:
            return -1, -1
    ...
    def __extract_links(self, soup):        
        ...
        if body != None:
            ...
            self.article_links = [ l for l in self.article_links if l["article_id"] != None ]
            self.article_links = [ l for l in self.article_links if l["article_id"] != self.article_id ]

    def __article_id_from_link(self, link):
        if "ref:odnb/" in link:
            return link.split("ref:odnb/")[1]
        
        if "odnb-9780198614128-e-" in link:
            link = link.split("odnb-9780198614128-e-")[1]
            return link.split("/")[0]

        if "https://doi.org/10.1093/odnb/" in link:            
            link = link.split("https://doi.org/10.1093/odnb/")[1]
            return link.split(".")[-1]
